...
# #新添加
# def custom_download_model(repo_id, cache_dir, filename):
...
# TODO: 修改默认参数；将远程连接hf的部分修改为读取本地缓存文件
@dataclass
class ModelArguments:
    #这里是否可以将原始的mistral-7B转换成本地的bonito?
    model_name_or_path: Optional[str] = field(default="mistralai/Mistral-7B-v0.1")
    # model_name_or_path: Optional[str] = field(default="./mistral_model")    #修改过的
...
class DataArguments:
    ...
    json_path: str = field(
        default=None,
        metadata={"help": "Path to JSON file containing training data with 'question' and 'output' fields."},
    )
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    num_train_epochs: Optional[int] = field(
        default=5, metadata={"help": "How many epochs to train for"}
    )
...
def get_accelerate_model(args, checkpoint_model_id_or_path, trainable=True):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        token = None,  # Set your HF token here or use HF_TOKEN env variable
    )
    ...
    if not args.full_finetune:
        model = prepare_model_for_kbit_training(
            model, use_gradient_checkpointing=args.gradient_checkpointing   #修改过的
    ...
    for name, module in model.named_modules():
        ...
        #补充
        if "mlp.down_proj.lora_B" in name:
            if hasattr(module, "gradient_checkpointing"):
                module.gradient_checkpointing = False
            print(f"Disabled gradient checkpointing for {name}")
        
        if hasattr(module, "lora_A") or hasattr(module, "lora_B"):
            # module.lora_A = copy.deepcopy(module.lora_A)
            # module.lora_B = copy.deepcopy(module.lora_B)
            print(f"Deep-copied LoRA weights for {name}")
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
        ...
        if args.training_type == "instruct":
            ...
            if args.json_path is not None:
                ...
                dataset = Dataset.from_dict({
                    ...
                    "type": ["qa" for _ in range(len(data))],
                    "input": [item["input"] for item in data],
                    "output": [item["output"] for item in data],
                })
                return dataset
            elif args.custom_dataset_path is not None:
                ...
                #修改
                elif args.supervision_source == "dapt_self":
                    #这里修改数据源就可以
                    dataset_path = "./data/fixed_data.json"
                    with open(dataset_path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    
                    # 使用question作为input，output保持不变
                    dataset = Dataset.from_dict({
                            "id": [str(i) for i in range(len(data))],
                            "type": ["qa" for _ in range(len(data))],
                ...
                #     return load_dataset("BatsResearch/bonito-experiment", config_name)
                    ...
                    #不是p3、dapt，则用bonito得到的合成数据作为训练数据微调模型
    ...
    def format_dataset(dataset, add_prompts=True):
        ...
        def preprocess_function(examples):
            ...
            outputs = [example.strip() for example in examples["output"]]
        ...
        def preprocess_mistral_instruct_function(examples):
            ...
            inputs = [
                f"[INST] {example.strip()} [/INST] " for example in examples["input"]
            ]
            outputs = [example.strip() for example in examples["output"]]
        ...
        if add_prompts:
            #这里进行了路径的修改
            # if args.model_name_or_path == "mistralai/Mistral-7B-Instruct-v0.2":     #修改过的
            if args.model_name_or_path == "mistral_model/models--mistralai--Mistral-7B-v0.1/snapshots/7231864981174d9bee8c7687c24c8344414eae6b":
...
def train():
    ...
    checkpoint_model_id_or_path = args.checkpoint_model_id_or_path     #默认不使用checkpoint
    #在此报错→√
    ...
    if args.do_train:
        ...
        logger.info("*** Train ***")
        #打印batch信息看为什么一个epoch是156
        print(f"Batch size per device: {training_args.per_device_train_batch_size}")
        print(f"Total steps: {len(trainer.get_train_dataloader())}")
