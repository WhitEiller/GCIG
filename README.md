...
We have integrated **ToG (Think-on-Graph)** [[ICLR 2024](https://arxiv.org/abs/2307.07697)] into our framework to enable sophisticated multi-hop reasoning capabilities in the **CoTAG (Chain-of-Thought-guided Answer Generation)** component. ToG provides graph-based reasoning that powers GCIG's ability to generate high-quality answers with interpretable reasoning chains.

#### ToG Core Capabilities in GCIG

**Algorithm Pipeline (Implemented in `instruct_generate/answer.py`):**
1. **Entity Linking** - Identifies topic entities from questions
2. **Iterative Relation Exploration** - Discovers relevant relations at configurable depth (default: 3)
3. **Entity Search & Pruning** - Filters candidate entities using:
...
**Key Features in GCIG:**
- Multi-hop reasoning over custom knowledge graphs built by EKG_construct
- Temperature-controlled exploration (0.4) vs. reasoning (0.0)
- Configurable reasoning depth (REASONING_DEPTH) and width (REASONING_WIDTH)
- Fallback to direct LLM inference when graph reasoning is unavailable
- Integration with BM25, SentenceBERT, and LLM-based ranking methods

#### Integration Architecture

ToG's reasoning capabilities are deeply integrated with the `instruct_generate/answer.py` module:

```python
# Example: Using ToG-enhanced reasoning in answer.py
from graphrag.query import generate

# Enable GraphRAG reasoning (powered by ToG)
USE_GRAPH_REASONING = True
REASONING_DEPTH = 3              # Multi-hop depth
REASONING_WIDTH = 3              # Candidate width per step

# GraphRAG generates answers with reasoning chains
context, answer = generate(
    query="What products are manufactured by organizations founded in California?",
    graph_dir=GRAPH_DIR,
    depth=REASONING_DEPTH,
    width=REASONING_WIDTH
)
```

**Integration Points:**
- **Entity Extraction:** ToG's entity linking used in question analysis
- **Subgraph Retrieval:** Multi-hop relation exploration from seed entities
- **Candidate Ranking:** LLM/BM25/SentenceBERT pruning methods from ToG
- **Answer Generation:** Chain-of-thought reasoning with graph context
- **Data Models:** Compatible with EKG_construct output format (entities.parquet, relations.parquet)

#### Configuration in CoTAG

Edit `instruct_generate/answer.py` to configure ToG-based reasoning:

```python
# GraphRAG Configuration (ToG-powered)
GRAPH_DIR = "/path/to/graph"     # EKG directory from EKG_construct
USE_GRAPH_REASONING = True       # Enable ToG reasoning
REASONING_DEPTH = 3              # Maximum exploration depth
REASONING_WIDTH = 3              # Candidates per step

# Pruning methods: "llm", "bm25", "sentencebert"
PRUNING_METHOD = "llm"
```
